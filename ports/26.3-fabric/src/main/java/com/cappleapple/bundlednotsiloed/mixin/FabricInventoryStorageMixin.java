...
@Mixin(value = ContainerStorage.class, remap = false)
public interface FabricInventoryStorageMixin {
    @Inject(method = "of", at = @At("RETURN"), cancellable = true, remap = false)
    private static void bns$logicalStorage(Container container, Direction direction, CallbackInfoReturnable<ContainerStorage> ci) {
