...
public final class StackReference {
    ...
    public Tag save(HolderLookup.Provider provider) {
        return unresolved == null ? ItemStack.CODEC.encodeStart(provider.createSerializationContext(net.minecraft.nbt.NbtOps.INSTANCE), prototype).getOrThrow() : unresolved.copy();
    }

    public static Optional<StackReference> load(HolderLookup.Provider provider, CompoundTag tag) {
        Optional<ItemStack> parsed = ItemStack.CODEC.parse(provider.createSerializationContext(net.minecraft.nbt.NbtOps.INSTANCE), tag).result().filter(stack -> !stack.isEmpty());
