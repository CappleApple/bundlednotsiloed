...
public final class BundledNotSiloedEmiPlugin implements EmiPlugin {
    private static final List<Identifier> COMPATIBLE_CRAFTING_MENU_TYPES = List.of(
            Identifier.fromNamespaceAndPath("visualworkbench", "crafting")
    );
    ...
    public void register(EmiRegistry registry) {
        ...
        for (Identifier id : COMPATIBLE_CRAFTING_MENU_TYPES) {
    ...
    private static EmiPlayerInventory fullInventory(List<Slot> craftingSlots) {
        ...
        ModAttachments.get(minecraft.player).inventory().backingStacks().stream()
    ...
    private static boolean transferable(EmiRecipe recipe, int gridWidth, int gridHeight) {
        ...
        return backing != null && backing.value() instanceof CraftingRecipe craftingRecipe
                && com.cappleapple.bundlednotsiloed.compat.RecipeTransferService.fits(craftingRecipe, gridWidth, gridHeight);
    }
    ...
    private static boolean canCraft(EmiRecipe recipe, EmiCraftContext<?> context) {
        ...
        if (minecraft.player == null || backing == null
                ...
                || !(context.getScreenHandler() instanceof net.minecraft.world.inventory.AbstractCraftingMenu recipeMenu)) return false;
    ...
    private static boolean transfer(EmiRecipe recipe, EmiCraftContext<?> context) {
        ...
        com.cappleapple.bundlednotsiloed.platform.PacketDistributor.sendToServer(new RecipeTransferPayload(
                backing.id().identifier(), context.getAmount() > 1, destination));
        return true;
    }
