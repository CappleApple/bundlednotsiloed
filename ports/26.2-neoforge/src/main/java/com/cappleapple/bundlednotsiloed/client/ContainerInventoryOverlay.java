...
public final class ContainerInventoryOverlay {
    ...
    public static void keyPressed(ScreenEvent.KeyPressed.Pre event) {
        if (event.getScreen() != activeScreen || activeGrid == null) return;
        if ((categoryMenuOpen || settingsMenuOpen) && event.getKeyCode() == com.mojang.blaze3d.platform.InputConstants.KEY_ESCAPE) {
            categoryMenuOpen = false;
            settingsMenuOpen = false;
        ...
        if (event.getKeyCode() == com.mojang.blaze3d.platform.InputConstants.KEY_A && net.minecraft.client.Minecraft.getInstance().hasControlDown()) {
            SEARCH.selectAll();
            event.setCanceled(true);
            return;
        }
