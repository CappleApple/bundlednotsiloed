...
public final class ContainerInventoryOverlay {
        ...
        if (activeGrid.sortContains(mouseX, mouseY)) {
            ...
            if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
                ...
                selectCategory(InventoryBrowserControls.currentCategory(
                        com.cappleapple.bundlednotsiloed.data.ModAttachments.get(minecraft.player)));
            }
            return true;
        }
    ...
    private static void restoreActiveScreen() {
        ...
        if (Minecraft.getInstance().player != null) {
            com.cappleapple.bundlednotsiloed.data.ModAttachments.get(Minecraft.getInstance().player).resetInventoryWindow();
        }
    ...
    private static void renderCategoryMenu(GuiGraphics graphics, int mouseX, int mouseY) {
        ...
        ResourceLocation selected = com.cappleapple.bundlednotsiloed.data.ModAttachments.get(Minecraft.getInstance().player).selectedCategoryPreference();
    ...
    private static void renderEntries(GuiGraphics graphics, int mouseX, int mouseY) {
        ...
        Minecraft minecraft = Minecraft.getInstance();
        long capacity = com.cappleapple.bundlednotsiloed.data.ModAttachments.get(minecraft.player).inventory().capacity();
    ...
    private static SortMode currentSort() {
        return com.cappleapple.bundlednotsiloed.data.ModAttachments.get(Minecraft.getInstance().player)
                .inventorySortPreference();
    }

    private static List<CategoryOption> categoryOptions() {
        return InventoryBrowserControls.categoryOptions(
                com.cappleapple.bundlednotsiloed.data.ModAttachments.get(Minecraft.getInstance().player));
    }
    ...
    private static void selectCategory(CategoryDefinition category) {
        var data = com.cappleapple.bundlednotsiloed.data.ModAttachments.get(Minecraft.getInstance().player);
    ...
    private static LogicalInventoryEntry aggregate(ItemStack stack) {
        Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.player == null) return null;
        return com.cappleapple.bundlednotsiloed.data.ModAttachments.get(minecraft.player).inventory().entriesAtOrAfter(9).stream()
    ...
    private static int visibleEntryCount() {
        ...
        int logicalMainSlots = com.cappleapple.bundlednotsiloed.data.ModAttachments.get(minecraft.player)
