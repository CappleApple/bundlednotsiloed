...
public final class ModCommands {
    ...
    private static int validate(CommandSourceStack source) throws com.mojang.brigadier.exceptions.CommandSyntaxException {
        DynamicCapacityInventory inventory = com.cappleapple.bundlednotsiloed.data.ModAttachments.get(source.getPlayerOrException()).inventory();
        ...
        source.sendSuccess(() -> Component.literal(valid ? "Inventory invariants are valid." : "Inventory invariant failure: " + errors.get(0)), false);
        return valid ? 1 : 0;
    }
    ...
    private static int resetCategories(CommandSourceStack source) throws com.mojang.brigadier.exceptions.CommandSyntaxException {
        ServerPlayer player = source.getPlayerOrException();
        CategoryPresetManager.reset(com.cappleapple.bundlednotsiloed.data.ModAttachments.get(player).categories());
