...
public final class InventoryCompatibilityGameTests {
    ...
    @GameTest(templateNamespace = BundledNotSiloed.MOD_ID, template = EMPTY_TEMPLATE)
    public static void clearVisitsHiddenLogicalStorage(GameTestHelper helper) {
        ...
        DynamicCapacityInventory inventory = com.cappleapple.bundlednotsiloed.data.ModAttachments.get(player).inventory();
        ...
        com.cappleapple.bundlednotsiloed.platform.GameTestAssertions.equal(helper, removed, 10, "/clear did not include hidden logical storage");
        ...
        com.cappleapple.bundlednotsiloed.platform.GameTestAssertions.equal(helper, inventory.syntheticStack(37).getCount(), 2,
    ...
    public static void creativePickAndPlacementUseLogicalStorage(GameTestHelper helper) {
        ...
        AttributeInstance capacity = player.getAttribute(ModAttributes.INVENTORY_CAPACITY.get());
        ...
        com.cappleapple.bundlednotsiloed.platform.GameTestAssertions.equal(helper, emptyPlayerSlot.getMaxStackSize(new ItemStack(Items.DIRT)), 64,
    ...
    public static void creativeCursorTransactionsUseHiddenStorage(GameTestHelper helper) {
        ...
        com.cappleapple.bundlednotsiloed.platform.GameTestAssertions.equal(helper, taken.getCount(), 4,
                "Creative cursor extraction did not take half of the backend stack");
        com.cappleapple.bundlednotsiloed.platform.GameTestAssertions.equal(helper, inventory.syntheticStack(36).getCount(), 3,
        ...
        com.cappleapple.bundlednotsiloed.platform.GameTestAssertions.equal(helper, stowed.acceptedAmount(), 5,
    ...
    public static void externalQuickMoveHonorsSelectedNewItemDestination(GameTestHelper helper) {
        ...
        com.cappleapple.bundlednotsiloed.platform.GameTestAssertions.equal(helper, inventory.syntheticStack(10).getCount(), 4,
        ...
        com.cappleapple.bundlednotsiloed.platform.GameTestAssertions.equal(helper, inventory.syntheticStack(36).getCount(), 13,
    ...
    public static void playerInventoryQuickMoveUsesDirectionBySource(GameTestHelper helper) {
        ...
        com.cappleapple.bundlednotsiloed.platform.GameTestAssertions.equal(helper, inventory.syntheticStack(0).getCount(), 64,
        ...
        com.cappleapple.bundlednotsiloed.platform.GameTestAssertions.equal(helper, inventory.syntheticStack(39).getCount(), 63,
    ...
    public static void recipeInventoryHooksDetectAndExtractStowedIngredients(GameTestHelper helper) {
        ...
        Recipe<?> holder = helper.getLevel().getRecipeManager()
                .byKey(new ResourceLocation("stick")).orElseThrow();
        helper.assertTrue(holder instanceof CraftingRecipe,
                "The vanilla stick recipe was not a crafting recipe");
        CraftingRecipe recipe = (CraftingRecipe)holder;
        ...
        com.cappleapple.bundlednotsiloed.platform.GameTestAssertions.equal(helper, sourceSlot, Integer.MAX_VALUE,
                "Recipe extraction exposed a window-dependent vanilla slot");
        ItemStack sourceSnapshot = player.getInventory().getItem(sourceSlot);
        com.cappleapple.bundlednotsiloed.platform.GameTestAssertions.equal(helper, sourceSnapshot.getCount(), 2,
        ...
        com.cappleapple.bundlednotsiloed.platform.GameTestAssertions.equal(helper, removed.getCount(), 2,
    ...
    private static Player migratedPlayer(GameTestHelper helper, GameType gameType) {
        Player player = gameType == GameType.SURVIVAL ? helper.makeMockSurvivalPlayer() : helper.makeMockPlayer();
        gameType.updatePlayerAbilities(player.getAbilities());
        com.cappleapple.bundlednotsiloed.data.ModAttachments.get(player).setMigratedVanillaInventory();
        return player;
    }
}
