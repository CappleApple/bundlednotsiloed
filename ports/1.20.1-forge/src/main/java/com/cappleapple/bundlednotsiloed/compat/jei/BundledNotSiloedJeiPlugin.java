...
public final class BundledNotSiloedJeiPlugin implements IModPlugin {
    ...
    private static final class UnifiedCraftingTransferHandler<C extends AbstractContainerMenu>
        ...
        public IRecipeTransferError transferRecipe(
                C menu, CraftingRecipe recipe, IRecipeSlotsView recipeSlots,
                Player player, boolean maxTransfer, boolean doTransfer
        ) {
            if (!(menu instanceof RecipeBookMenu<?> recipeMenu)) return helper.createInternalError();
            if (!recipe.canCraftInDimensions(recipeMenu.getGridWidth(), recipeMenu.getGridHeight())) {
                return helper.createUserErrorWithTooltip(Component.translatable("message.bundlednotsiloed.recipe_too_large"));
            }

            StackedContents contents = new StackedContents();
            com.cappleapple.bundlednotsiloed.data.ModAttachments.get(player).inventory().backingStacks().forEach(contents::accountSimpleStack);
            ...
            if (doTransfer) PacketDistributor.sendToServer(new RecipeTransferPayload(
                    recipe.getId(), maxTransfer, RecipeTransferDestination.NONE));
            return null;
        }
    }
}
