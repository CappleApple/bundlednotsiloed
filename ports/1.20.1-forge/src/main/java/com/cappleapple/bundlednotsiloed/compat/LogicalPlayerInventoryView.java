...
public final class LogicalPlayerInventoryView {
    ...
    public static int slotCount(Player player) {
        return slotCount(com.cappleapple.bundlednotsiloed.data.ModAttachments.get(player));
    }

    public static ItemStack stackInSlot(Player player, int logicalSlot) {
        return stackInSlot(com.cappleapple.bundlednotsiloed.data.ModAttachments.get(player), logicalSlot);
    }
