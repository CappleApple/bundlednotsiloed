...
public final class ContainerTransfers {
    ...
    public static int moveBrowserEntryToMenu(
        ...
        for (int operation = 0; operation < maximumOperations; operation++) {
            com.cappleapple.bundlednotsiloed.data.ModAttachments.get(player).refreshInventoryWindow();
    ...
    private static IItemHandler lookedAtHandler(ServerPlayer player) {
        ...
        return blockEntity == null ? null : blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER, blockHit.getDirection()).orElse(null);
    }
