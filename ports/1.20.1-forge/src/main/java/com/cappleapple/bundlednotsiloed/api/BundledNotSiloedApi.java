...
public final class BundledNotSiloedApi {
    ...
    public static ICapacityInventory inventory(Player player) {
        return com.cappleapple.bundlednotsiloed.data.ModAttachments.get(player).inventory();
    }

    public static List<CategoryView> categories(Player player) {
        return com.cappleapple.bundlednotsiloed.data.ModAttachments.get(player).categories().categories().stream()
