...
public final class StackReference {
    ...
    public boolean matches(ItemStack stack) {
        return !prototype.isEmpty() && ItemStack.isSameItemSameTags(prototype, stack);
    }

    public Tag save(HolderLookup.Provider provider) {
        return unresolved == null ? prototype.save(new CompoundTag()) : unresolved.copy();
    }

    public static Optional<StackReference> load(HolderLookup.Provider provider, CompoundTag tag) {
        Optional<ItemStack> parsed = java.util.Optional.of(ItemStack.of(tag)).filter(stack -> !stack.isEmpty());
