# Bundled Not Siloed: 26.3-fabric

Branch: `cappleapple/26.3-fabric`. This branch contains only the `26.3-fabric` port. The original NeoForge 1.21.1 implementation remains at the repository root.
...
- Requires Stacks Not Slots 1.1 from [the matching branch](https://github.com/CappleApple/stacksnotslots/tree/cappleapple/26.3-fabric).

## Install

Install `bundlednotsiloed-26.3-fabric-1.5.1.jar` and `stacksnotslots-26.3-fabric-1.1.jar` on the client and server. Use only the pair for this Minecraft version and loader. Fabric additionally requires Fabric API.

## Build

Keep `bundled-not-siloed` and `stacks-not-slots` as sibling directories, both checked out on `cappleapple/26.3-fabric`. The dependency's default branch contains the original library, not this target.

From the Bundled Not Siloed repository root:

```powershell
.\ports\build-port.ps1 26.3-fabric
```

```bash
bash ports/build-port.sh 26.3-fabric
```

The helper builds and tests the matching dependency first. Production JARs are written to `ports/26.3-fabric/build/libs/` in each repository. Do not install `-sources.jar` or Forge `-dev.jar` artifacts.

```powershell
.\ports\26.3-fabric\gradlew.bat -p ports/26.3-fabric runGameTestServer
```
