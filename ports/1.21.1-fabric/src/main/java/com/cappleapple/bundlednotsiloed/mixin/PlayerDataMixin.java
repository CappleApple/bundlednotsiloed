...
public abstract class PlayerDataMixin implements PlayerDataAccess {
 ...
 private void bns$save(CompoundTag tag,CallbackInfo ci) { tag.put("bundlednotsiloed:player_inventory",bns$inventoryData().serializeNBT(((Player)(Object)this).registryAccess())); }
 @Inject(method="readAdditionalSaveData",at=@At("TAIL"))
 private void bns$load(CompoundTag tag,CallbackInfo ci) {
  if(tag.contains("bundlednotsiloed:player_inventory",10)) bns$inventoryData().deserializeNBT(((Player)(Object)this).registryAccess(),tag.getCompound("bundlednotsiloed:player_inventory"));
 }
}
