...
public final class ContainerInventoryOverlay {
    ...
    private static int suppressedTypedKey = com.mojang.blaze3d.platform.InputConstants.UNKNOWN.getValue();
    ...
    private static Identifier cachedCategory;
    ...
    public static void initialize(UiEvent event) {
    ...
    public static void render(UiEvent event) {
    ...
    public static void renderTooltip(UiEvent event) {
        ...
        if (hoveredCategory != null) {
            graphics.setTooltipForNextFrame(Minecraft.getInstance().font, hoveredCategory.name(),
            ...
            InventoryButtonTooltipRenderer.render(graphics, Minecraft.getInstance().font,
                    hoveredTooltip, event.getMouseX(), event.getMouseY(), net.minecraft.client.Minecraft.getInstance().hasShiftDown());
        }
        graphics.pose().popMatrix();
    }
    ...
    public static boolean mouseButton(int button, int action) {
        ...
        if (!(minecraft.gui.screen() instanceof AbstractContainerScreen<?> screen) || !supports(screen)
                || minecraft.player == null) return false;
        prepare(screen);
        if (activeGrid == null) return false;
        if (action == com.mojang.blaze3d.platform.InputConstants.RELEASE) {
            if (button != consumedReleaseButton) return false;
            consumedReleaseButton = -1;
            return true;
        }
        if (action != com.mojang.blaze3d.platform.InputConstants.PRESS) return false;
        ...
        if (activeGrid.searchContains(mouseX, mouseY)) {
            ...
            if (button == com.mojang.blaze3d.platform.InputConstants.MOUSE_BUTTON_RIGHT) {
                ...
                clearSearchFocus();
            } else if (button == com.mojang.blaze3d.platform.InputConstants.MOUSE_BUTTON_LEFT) {
        ...
        if (activeGrid.categoryContains(mouseX, mouseY)) {
            consumedReleaseButton = button;
            clearSearchFocus();
            if (button == com.mojang.blaze3d.platform.InputConstants.MOUSE_BUTTON_LEFT && net.minecraft.client.Minecraft.getInstance().hasShiftDown()) {
                com.cappleapple.bundlednotsiloed.platform.PacketDistributor.sendToServer(new StowMainGridPayload());
        ...
        if (activeGrid.sortContains(mouseX, mouseY)) {
            ...
            if (button == com.mojang.blaze3d.platform.InputConstants.MOUSE_BUTTON_LEFT) {
                ...
                selectCategory(InventoryBrowserControls.currentCategory(
                        ModAttachments.get(minecraft.player)));
            }
            return true;
        }
        ...
        if (cell < 0 || (button != com.mojang.blaze3d.platform.InputConstants.MOUSE_BUTTON_LEFT
                && button != com.mojang.blaze3d.platform.InputConstants.MOUSE_BUTTON_RIGHT)) return false;
        if (net.minecraft.client.Minecraft.getInstance().hasShiftDown() && button == com.mojang.blaze3d.platform.InputConstants.MOUSE_BUTTON_RIGHT
            ...
            consumedReleaseButton = button;
            com.cappleapple.bundlednotsiloed.platform.PacketDistributor.sendToServer(new BrowserTransferPayload(
    ...
    public static boolean mouseScrolled(double deltaY) {
        Minecraft minecraft = Minecraft.getInstance();
        if (!(minecraft.gui.screen() instanceof AbstractContainerScreen<?> screen) || !supports(screen)) return false;
        prepare(screen);
        if (activeGrid == null) return false;
        double mouseX = scaledMouseX(minecraft);
        double mouseY = scaledMouseY(minecraft);
    ...
    public static void keyPressed(UiEvent event) {
        if (event.getScreen() != activeScreen || activeGrid == null) return;
        if ((categoryMenuOpen || settingsMenuOpen) && event.getKey() == com.mojang.blaze3d.platform.InputConstants.KEY_ESCAPE) {
            categoryMenuOpen = false;
            settingsMenuOpen = false;
        ...
        if (!searchFocused && hoveredEntry != null
                && Minecraft.getInstance().options.keyDrop.matches(event.getKeyEvent())) {
            com.cappleapple.bundlednotsiloed.platform.PacketDistributor.sendToServer(new InventoryActionPayload(
                    net.minecraft.client.Minecraft.getInstance().hasControlDown() ? InventoryActionPayload.Action.DROP_STACK
        ...
        if (!searchFocused && (net.minecraft.client.Minecraft.getInstance().hasShiftDown() && ClientKeyMappings.SEARCH_BROWSER.matches(event.getKeyEvent()))) {
        ...
        if (Minecraft.getInstance().options.keyInventory.matches(event.getKeyEvent())) {
            event.setCanceled(true);
            return;
        }
        if (event.getKey() == com.mojang.blaze3d.platform.InputConstants.KEY_A && net.minecraft.client.Minecraft.getInstance().hasControlDown()) {
            SEARCH.selectAll();
            event.setCanceled(true);
            return;
        }
        switch (event.getKey()) {
            case com.mojang.blaze3d.platform.InputConstants.KEY_ESCAPE -> {
                clearSearchFocus();
            }
            case com.mojang.blaze3d.platform.InputConstants.KEY_BACKSPACE -> updateSearch(SEARCH.backspace());
            case com.mojang.blaze3d.platform.InputConstants.KEY_DELETE -> updateSearch(SEARCH.deleteSelection());
            default -> { }
        }
        event.setCanceled(true);
    }
    ...
    public static void keyReleased(UiEvent event) {
        if (event.getKeyCode() == suppressedTypedKey) suppressedTypedKey = com.mojang.blaze3d.platform.InputConstants.UNKNOWN.getValue();
    }

    public static void characterTyped(UiEvent event) {
        if (event.getScreen() != activeScreen || !searchFocused) return;
        if (suppressedTypedKey != com.mojang.blaze3d.platform.InputConstants.UNKNOWN.getValue()) {
            suppressedTypedKey = com.mojang.blaze3d.platform.InputConstants.UNKNOWN.getValue();
            event.setCanceled(true);
            return;
        }
    ...
    private static void restoreActiveScreen() {
        ...
        if (Minecraft.getInstance().player != null) {
            ModAttachments.get(Minecraft.getInstance().player).resetInventoryWindow();
        }
    ...
    private static PlayerGrid findPlayerGrid(AbstractContainerScreen<?> screen) {
        ...
        return new PlayerGrid(List.copyOf(cells), ((com.cappleapple.bundlednotsiloed.mixin.ContainerScreenAccessor)screen).bns$left(), ((com.cappleapple.bundlednotsiloed.mixin.ContainerScreenAccessor)screen).bns$top());
    }
    ...
    private static void renderToolbar(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        Minecraft minecraft = Minecraft.getInstance();
        ...
        InventoryBrowserControls.renderButton(graphics,
                ...
                net.minecraft.client.Minecraft.getInstance().hasShiftDown() ? new ItemStack(Items.STICKY_PISTON)
        ...
        if (searchHovered) {
            hoveredTooltip = InventorySearchBar.tooltip(data.inventory(), net.minecraft.client.Minecraft.getInstance().hasShiftDown());
            ...
            hoveredTooltip = net.minecraft.client.Minecraft.getInstance().hasShiftDown()
            ...
            hoveredTooltip = InventoryBrowserControls.sortTooltip(net.minecraft.client.Minecraft.getInstance().hasShiftDown());
        }
    }

    private static void renderCategoryMenu(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        Identifier selected = ModAttachments.get(Minecraft.getInstance().player).selectedCategoryPreference();
        for (int offset = 0; offset < layout.visibleCount(); offset++) {
            ...
            graphics.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, INVENTORY_TEXTURE, x, y, 7, 83,
                    InventoryScreenLayout.CELL_SIZE, InventoryScreenLayout.CELL_SIZE, 256, 256);
            ...
            graphics.item(option.icon(), x + 1, y + 1);
    ...
    private static boolean isSearchBlockingTarget(double mouseX, double mouseY) {
        ...
        for (Slot slot : activeScreen.getMenu().slots) {
            if (InventoryScreenLayout.inside(mouseX, mouseY,
                    ((com.cappleapple.bundlednotsiloed.mixin.ContainerScreenAccessor)activeScreen).bns$left() + slot.x - 1,
                    ((com.cappleapple.bundlednotsiloed.mixin.ContainerScreenAccessor)activeScreen).bns$top() + slot.y - 1, 18, 18)) {
                return true;
            }
        }
        return false;
    ...
    private static void renderSettingsMenu(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
    ...
    private static void renderEntries(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        if (!identityWindow) {
            renderScrollbar(graphics, visibleEntryCount());
            return;
        }
        Minecraft minecraft = Minecraft.getInstance();
        long capacity = ModAttachments.get(minecraft.player).inventory().capacity();
        for (Cell cell : activeGrid.cells()) {
            ...
            graphics.item(entry.representative(), x, y);
    ...
    private static void renderScrollbar(GuiGraphicsExtractor graphics, int entryCount) {
    ...
    private static List<LogicalInventoryEntry> entries() {
        ...
        var data = ModAttachments.get(minecraft.player);
        Identifier categoryId = data.selectedCategoryPreference();
    ...
    private static SortMode currentSort() {
        return ModAttachments.get(Minecraft.getInstance().player)
    ...
    private static void selectCategory(CategoryDefinition category) {
        ...
        Identifier selected = category == null ? null : category.id();
        data.setSelectedCategoryPreference(selected);
        data.setInventorySortPreference(sort);
        com.cappleapple.bundlednotsiloed.platform.PacketDistributor.sendToServer(new InventoryViewPreferencesPayload(sort, selected));
        scrollRow = 0;
        invalidateEntries();
        ensureInventoryWindow(true);
    }
    ...
    private static void cycleSort(int direction) {
        ...
        com.cappleapple.bundlednotsiloed.platform.PacketDistributor.sendToServer(new InventoryViewPreferencesPayload(
    ...
    private static void clickSettingsRow(AbstractContainerScreen<?> parent, int row) {
        ...
        settingsMenuOpen = false;
        switch (row) {
            case 0 -> minecraft.gui.setScreen(new CategoryManagerScreen(parent, minecraft.player));
            case 1 -> {
                cycleSort(1);
                settingsMenuOpen = true;
            }
            case 2 -> minecraft.gui.setScreen(new InventoryBrowserSettingsScreen(parent));
            default -> { }
        }
    }
    ...
    private static LogicalInventoryEntry aggregate(ItemStack stack) {
        Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.player == null) return null;
        return ModAttachments.get(minecraft.player).inventory().entriesAtOrAfter(9).stream()
    ...
    private static int visibleEntryCount() {
        ...
        int logicalMainSlots = ModAttachments.get(minecraft.player)
