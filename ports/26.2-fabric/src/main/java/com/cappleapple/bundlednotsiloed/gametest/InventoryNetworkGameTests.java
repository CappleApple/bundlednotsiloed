...
public final class InventoryNetworkGameTests {
    ...
    public void identicalVanillaWritesKeepLiveItemReferences(GameTestHelper helper) {
        ...
        player.getInventory().getNonEquipmentItems().set(33, live.copy());
        data.reconcileVanillaCompatibilityView();
        helper.assertTrue(player.getInventory().getNonEquipmentItems().get(33) == live, "List reconciliation replaced the live item object");
        helper.succeed();
    }

    @GameTest(structure = "fabric-gametest-api-v1:empty")
    public void sophisticatedHoverContextAndOpenItemStayStable(GameTestHelper helper) throws ReflectiveOperationException {
        ...
        var data = ModAttachments.get(player);
        data.setMigratedVanillaInventory();
        ItemStack backpack = new ItemStack(BuiltInRegistries.ITEM.getValue(
                Identifier.parse("sophisticatedbackpacks:netherite_backpack")));
        ...
        ItemStack child = new ItemStack(BuiltInRegistries.ITEM.getValue(Identifier.parse("sophisticatedbackpacks:backpack")));
