...
public final class ModCommands {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("bundlednotsiloed")
                ...
                .then(Commands.literal("categories")
                        ...
                        .then(Commands.literal("reload").requires(source -> source.permissions().hasPermission(net.minecraft.server.permissions.Permissions.COMMANDS_GAMEMASTER)).executes(context -> reloadCategories(context.getSource())))));
    }
    ...
    private static int capacity(CommandSourceStack source, ServerPlayer player) {
        ...
        source.sendSuccess(() -> Component.literal(player.getGameProfile().name() + ": " + used.decimalString() + " / " + inventory.capacity()
    ...
    private static int validate(CommandSourceStack source) throws com.mojang.brigadier.exceptions.CommandSyntaxException {
        DynamicCapacityInventory inventory = ModAttachments.get(source.getPlayerOrException()).inventory();
    ...
    private static int resetCategories(CommandSourceStack source) throws com.mojang.brigadier.exceptions.CommandSyntaxException {
        ServerPlayer player = source.getPlayerOrException();
        CategoryPresetManager.reset(ModAttachments.get(player).categories());
