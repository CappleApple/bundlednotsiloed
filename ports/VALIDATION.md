# Validation: 1.20.1-forge - 2026-09-20
...
| --- | ---: | ---: | ---: | --- | --- |
| 1.20.1-forge | 111 | 33 | 11 | Passed | Passed |

This target passed 144 unit tests and 11 required GameTests. GameTest counts include the vanilla baseline where registered. The original NeoForge 1.21.1 project was also rebuilt: 111 unit tests and 8 required GameTests passed.
...
Build the matching dependency first. Place a disposable world at `ports/1.20.1-forge/run-client-smoke/saves/BNS-Audit/`; the `level.dat` file must be directly inside that directory. The gameplay fixture replaces the test player's inventory, so use a copied test world. From the target directory, run:
