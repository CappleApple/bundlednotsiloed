# Validation: 26.2-fabric - 2026-09-20
...
| --- | ---: | ---: | ---: | --- | --- |
| 26.2-fabric | 111 | 35 | 13 | Passed | Passed |

This target passed 146 unit tests and 13 required GameTests. GameTest counts include the vanilla baseline where registered. The original NeoForge 1.21.1 project was also rebuilt: 111 unit tests and 8 required GameTests passed.
...
Build the matching dependency first. Place a disposable world at `ports/26.2-fabric/run-client-smoke/saves/BNS-Audit/`; the `level.dat` file must be directly inside that directory. The gameplay fixture replaces the test player's inventory, so use a copied test world. From the target directory, run:
