...
public final class ContainerInventoryOverlay {
    ...
    public static void render(UiEvent event) {
        ...
        graphics.pose().translate(-((com.cappleapple.bundlednotsiloed.mixin.ContainerScreenAccessor)screen).bns$left(), -((com.cappleapple.bundlednotsiloed.mixin.ContainerScreenAccessor)screen).bns$top(), 100);
    ...
    public static void keyPressed(UiEvent event) {
        ...
        if (!searchFocused && (net.minecraft.client.gui.screens.Screen.hasShiftDown() && ClientKeyMappings.SEARCH_BROWSER.matches(pressed.getValue(), -1))) {
    ...
    private static void renderCategoryMenu(GuiGraphics graphics, int mouseX, int mouseY) {
        ...
        ResourceLocation selected = ModAttachments.get(Minecraft.getInstance().player).selectedCategoryPreference();
    ...
    private static int thumbHeight(int entryCount) {
        int totalRows = Math.max(InventoryScreenLayout.GRID_ROWS,
                com.cappleapple.bundlednotsiloed.platform.LegacyMath.ceilDiv(Math.max(0, entryCount), InventoryScreenLayout.GRID_COLUMNS));
    ...
    private static LogicalInventoryEntry aggregate(ItemStack stack) {
        ...
        return ModAttachments.get(minecraft.player).inventory().entriesAtOrAfter(9).stream()
                .filter(entry -> ItemStack.isSameItemSameTags(entry.representative(), stack))
                .findFirst()
                .orElse(null);
    }
    ...
    private record PlayerGrid(List<Cell> cells, int guiLeft, int guiTop) {
        int gridLeft() { return guiLeft + cells.get(0).x(); }
        int gridTop() { return guiTop + cells.get(0).y(); }
        int gridHeight() {
            int bottom = cells.stream().mapToInt(Cell::y).max().orElse(cells.get(0).y()) + 18;
            return bottom - cells.get(0).y();
        }
        int gridWidth() {
            int left = cells.stream().mapToInt(Cell::x).min().orElse(cells.get(0).x());
            int right = cells.stream().mapToInt(Cell::x).max().orElse(cells.get(0).x()) + 18;
            return right - left;
        }
        ...
        int scrollbarX() {
            return guiLeft + cells.stream().mapToInt(Cell::x).max().orElse(cells.get(0).x()) + 19;
        }
