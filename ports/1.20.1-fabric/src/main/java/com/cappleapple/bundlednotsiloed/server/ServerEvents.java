...
public final class ServerEvents {
    ...
    public static void playerLogin(ServerPlayer player) {
        if (!net.fabricmc.fabric.api.networking.v1.ServerPlayNetworking.canSend(player,
                com.cappleapple.bundlednotsiloed.network.InventorySnapshotPayload.TYPE.id())) {
            player.connection.disconnect(net.minecraft.network.chat.Component.literal("This server requires Bundled Not Siloed for Minecraft 1.20.1 Fabric."));
            return;
        }
    ...
    public static void clonePlayer(ServerPlayer oldPlayer, ServerPlayer newPlayer) {
        ...
        replacement.deserializeNBT(newPlayer.level().registryAccess(), original.serializeNBT(oldPlayer.level().registryAccess()));
        replacement.loadCustomization(newPlayer.level().registryAccess(), original.saveCustomization(oldPlayer.level().registryAccess()));
    }
