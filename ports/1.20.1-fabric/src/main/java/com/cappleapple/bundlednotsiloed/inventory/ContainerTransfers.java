...
public final class ContainerTransfers {
    ...
    public static int moveBrowserEntryToMenu(
        ...
        for (int operation = 0; operation < maximumOperations; operation++) {
            ModAttachments.get(player).refreshInventoryWindow();
    ...
    private static int moveVisibleEntryWithNativeQuickMove(ServerPlayer player, ItemStack prototype,
        ...
        for (int menuIndex = 0; menuIndex < menu.slots.size(); menuIndex++) {
            ...
            if (!isPlayerSlot(slot, player) || !slot.isActive() || !slot.hasItem()
                    ...
                    || !ItemStack.isSameItemSameTags(slot.getItem(), prototype)) continue;
    ...
    static long quantityAtOrAfter(
        ...
        for (int slot = Math.max(0, minimumSlot); slot < stacks.size(); slot++) {
            ...
            if (ItemStack.isSameItemSameTags(stack, prototype)) quantity += stack.getCount();
        }
        return quantity;
    }
    ...
    private static List<TransferredStack> moveOpenMenuToPlayer(ServerPlayer player) {
        ...
        try {
            for (int index = 0; index < menu.slots.size(); index++) {
                ...
                int amount = ItemStack.isSameItemSameTags(before, after) ? before.getCount() - after.getCount() : before.getCount();
    ...
    private static List<TransferredStack> movePlayerToLookedAt(ServerPlayer player) {
        Storage<ItemVariant> storage = lookedAtHandler(player);
        if (storage == null) return List.of();
        DynamicCapacityInventory inventory = ModAttachments.get(player).inventory();
        InventoryTransaction backend = new InventoryTransaction(ModAttachments.get(player));
        TransferAccumulator moved = new TransferAccumulator();
        for (ItemStack owned : dumpableStacks(inventory)) {
            if (com.cappleapple.bundlednotsiloed.compat.OpenBackpackGuard.protects(player, owned)) continue;
            try (Transaction transaction = Transaction.openOuter()) {
                int accepted = (int)storage.insert(ItemVariant.of(owned), owned.getCount(), transaction);
                if (accepted <= 0) continue;
                int extracted = backend.mutate(transaction, () -> inventory.extractAtOrAfter(owned, accepted, MAIN_GRID_START, false).extractedAmount());
    ...
    private static List<TransferredStack> moveLookedAtToPlayer(ServerPlayer player) {
        ...
        for (var view : storage) {
            while (!view.isResourceBlank() && operations++ < MAX_WORLD_STACK_OPERATIONS) {
                ItemVariant resource = view.getResource();
                ItemStack available = resource.toStack((int)Math.min(Integer.MAX_VALUE, view.getAmount()));
                int accepted = InventoryTransactions.insertIntoBackend(player, available, true).acceptedAmount();
                if (accepted <= 0) break;
                try (Transaction transaction = Transaction.openOuter()) {
                    int extracted = (int)view.extract(resource, accepted, transaction);
                    if (extracted <= 0) break;
                    int inserted = backend.mutate(transaction, () -> InventoryTransactions.insertIntoBackend(player, resource.toStack(extracted), false).acceptedAmount());
                    if (inserted != extracted) break;
                    transaction.commit();
                    moved.add(available, inserted);
                }
            }
            if (operations >= MAX_WORLD_STACK_OPERATIONS) break;
        }
        return moved.values();
    }

    private static Storage<ItemVariant> lookedAtHandler(ServerPlayer player) {
        HitResult hit = player.pick((player.getAbilities().instabuild ? 5.0D : 4.5D), 0.0F, false);
        ...
        return ItemStorage.SIDED.find(player.level(), blockHit.getBlockPos(), blockHit.getDirection());
    }
    ...
    private static final class TransferAccumulator {
        ...
        private void add(ItemStack stack, long amount) {
            for (int index = 0; index < values.size(); index++) {
                ...
                if (!ItemStack.isSameItemSameTags(current.prototype, stack)) continue;
