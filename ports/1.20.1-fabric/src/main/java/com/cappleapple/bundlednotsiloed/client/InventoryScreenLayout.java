...
public final class InventoryScreenLayout {
    ...
    public static int maximumScrollRow(int entryCount) {
        return Math.max(0, com.cappleapple.bundlednotsiloed.platform.LegacyMath.ceilDiv(Math.max(0, entryCount), GRID_COLUMNS) - GRID_ROWS);
    }

    public static int scrollbarThumbHeight(int entryCount) {
        int totalRows = Math.max(GRID_ROWS, com.cappleapple.bundlednotsiloed.platform.LegacyMath.ceilDiv(Math.max(0, entryCount), GRID_COLUMNS));
