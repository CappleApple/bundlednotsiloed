...
public final class ClientGameplayTest {
            ...
            switch (stage++) {
                case 0 -> {
                    ...
                    server(client, p -> {
                        ...
                        var data = p.getData(ModAttachments.PLAYER_DATA);
                ...
                case 2 -> {
                    ...
                    BundledNotSiloed.LOGGER.info("BNS_SEARCH_INPUT focused={} query={}", field(inventory,"searchFocused"), ((BrowserSearchQuery)field(inventory,"search")).value());
                }
                case 3 -> {
                    screenshot(client, "02-search");
                    check((boolean)field(inventory,"identityWindow"), "Search did not switch to filtered inventory; query="+((BrowserSearchQuery)field(inventory,"search")).value()+" dirty="+field(inventory,"windowDirty"));
                    ...
                    screenshot(client, "02-search");
                    client.gameMode.handleContainerInput(client.player.containerMenu.containerId, 9, 0, ContainerInput.PICKUP, client.player);
                }
                case 4 -> {
                    ...
                    client.gameMode.handleContainerInput(client.player.containerMenu.containerId, 36, 0, ContainerInput.PICKUP, client.player);
                }
                case 5 -> {
                ...
                case 15 -> {
                    check(client.player.getData(ModAttachments.PLAYER_DATA).categories().categories().stream().anyMatch(c->c.displayName().startsWith("Audit ")),"Saved category did not synchronize");
                    server(client,p->check(p.getData(ModAttachments.PLAYER_DATA).categories().categories().stream().anyMatch(c->c.displayName().startsWith("Audit ")),"Server did not store category edit"));
                    setScreen(client,new InventoryScreen(client.player));
                }
                case 16 -> {
                    inventory=(BundledInventoryScreen)screen(client);
                ...
                case 17 -> {
                    ...
                    server(client,p -> {var d=p.getData(ModAttachments.PLAYER_DATA);d.inventory().replaceSyntheticSlotFromItemUse(1,new ItemStack(Items.OAK_LOG));d.syncVanillaCompatibilityView();ModNetwork.queueInventorySync(p);});
                }
                case 18 -> nativeClick(client,37,ContainerInput.PICKUP);
                case 19 -> nativeClick(client,1,ContainerInput.PICKUP);
                case 20 -> {
                    check(client.player.containerMenu.getSlot(0).getItem().is(Items.OAK_PLANKS),"Crafting result did not synchronize");
                    screenshot(client,"12-crafting"); nativeClick(client,0,ContainerInput.QUICK_MOVE);
                }
                case 21 -> {
                    ...
                    nativeClick(client,36,ContainerInput.PICKUP);
                }
                case 22 -> nativeClick(client,45,ContainerInput.PICKUP);
                case 23 -> {
                    ...
                    nativeClick(client,45,ContainerInput.PICKUP);
                }
                case 24 -> nativeClick(client,36,ContainerInput.PICKUP);
                case 25 -> {
                ...
                case 26 -> {
                    ...
                    screenshot(client,"13-container"); nativeClick(client,0,ContainerInput.QUICK_MOVE);
                }
                case 27 -> {
    ...
    private static void resume(Minecraft client) throws Exception {
        switch(stage++) {
            case 0 -> {
                ...
                check(client.player.getData(ModAttachments.PLAYER_DATA).categories().categories().stream().anyMatch(c->c.displayName().startsWith("Audit ")),"Edited category did not persist across restart");
    ...
    private static void localization(Minecraft client) throws Exception {
        ...
        var location=net.minecraft.resources.Identifier.fromNamespaceAndPath("bundlednotsiloed","lang/en_us.json");
    ...
    private static long count(Minecraft client,net.minecraft.world.item.Item item) {return client.player.getData(ModAttachments.PLAYER_DATA).inventory().backingStacks().stream().filter(s->s.is(item)).mapToLong(ItemStack::getCount).sum();}
    private static long countAll(Minecraft client) {return client.player.getData(ModAttachments.PLAYER_DATA).inventory().backingStacks().stream().mapToLong(ItemStack::getCount).sum();}
    ...
    private static void press(Button button) throws Exception {button.onPress(new net.minecraft.client.input.KeyEvent(257,0,0));}
    private static Screen screen(Minecraft client) {return client.gui.screen();}
    private static void setScreen(Minecraft client,Screen value) {client.gui.setScreen(value);}
    private static void click(Screen screen,double x,double y,int button) {screen.mouseClicked(new net.minecraft.client.input.MouseButtonEvent(x,y,new net.minecraft.client.input.MouseButtonInfo(button == 0 ? com.mojang.blaze3d.platform.InputConstants.MOUSE_BUTTON_LEFT : com.mojang.blaze3d.platform.InputConstants.MOUSE_BUTTON_RIGHT,0)),false);}
    private static void type(Screen screen,String text) {for(int point:text.codePoints().toArray()){screen.charTyped(new net.minecraft.client.input.CharacterEvent(point));}}
    private static void scroll(Screen screen,double x,double y,double amount) {screen.mouseScrolled(x,y,0,amount);}
    private static void screenshot(Minecraft client,String name) throws Exception {Screenshot.grab(client.gameDirectory,name+".png",client.gameRenderer.mainRenderTarget(),1,message -> BundledNotSiloed.LOGGER.info("BNS_SCREENSHOT {}",message.getString()));}
    private static void nativeClick(Minecraft client,int slot,ContainerInput kind) {client.gameMode.handleContainerInput(client.player.containerMenu.containerId,slot,0,kind,client.player);}
