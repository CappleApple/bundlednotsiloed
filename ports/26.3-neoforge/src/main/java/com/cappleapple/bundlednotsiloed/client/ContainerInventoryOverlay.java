...
public final class ContainerInventoryOverlay {
    ...
    public static void render(ScreenEvent.Render.Foreground event) {
        ...
        // Foreground extraction uses screen coordinates before the carried item and tooltips.
    ...
    public static boolean mouseButton(int button, int action) {
        ...
        if (activeGrid.categoryContains(mouseX, mouseY)) {
            ...
            if (button == com.mojang.blaze3d.platform.InputConstants.MOUSE_BUTTON_LEFT && net.minecraft.client.Minecraft.getInstance().hasShiftDown()) {
                net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(new StowMainGridPayload());
        ...
        if (net.minecraft.client.Minecraft.getInstance().hasShiftDown() && button == com.mojang.blaze3d.platform.InputConstants.MOUSE_BUTTON_RIGHT
            ...
            consumedReleaseButton = button;
            net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(new BrowserTransferPayload(
    ...
    public static void keyPressed(ScreenEvent.KeyPressed.Pre event) {
        ...
        if (!searchFocused && hoveredEntry != null
            ...
            net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(new InventoryActionPayload(
        ...
        if (!searchFocused && ClientKeyMappings.SEARCH_BROWSER.isActiveAndMatches(pressed)) {
            suppressedTypedKey = event.getKey();
            Minecraft minecraft = Minecraft.getInstance();
    ...
    public static void keyReleased(ScreenEvent.KeyReleased.Pre event) {
        if (event.getKey() == suppressedTypedKey) suppressedTypedKey = com.mojang.blaze3d.platform.InputConstants.UNKNOWN.getValue();
    }
    ...
    private static void renderCategoryMenu(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        Identifier selected = Minecraft.getInstance().player
    ...
    private static void selectCategory(CategoryDefinition category) {
        ...
        net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(new InventoryViewPreferencesPayload(sort, selected));
        scrollRow = 0;
        invalidateEntries();
        ensureInventoryWindow(true);
    }
    ...
    private static void cycleSort(int direction) {
        ...
        net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(new InventoryViewPreferencesPayload(
