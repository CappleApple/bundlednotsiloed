...
public final class PlayerCategoryData {
    ...
    public CategoryDefinition find(Identifier id) {
    ...
    public void load(CompoundTag root) {
        categories.clear();
        initializedFromDefaults = root.getBooleanOr("InitializedFromDefaults", false);
        ListTag list = root.getListOrEmpty("Categories");
        for (int i = 0; i < list.size(); i++) {
            CategoryDefinition definition = loadCategory(list.getCompoundOrEmpty(i));
    ...
    public static CategoryDefinition loadCategory(CompoundTag tag) {
        ...
        Identifier icon = Identifier.tryParse(tag.getStringOr("Icon", ""));
        if (id == null || icon == null) return null;
        SortMode sort;
        try { sort = SortMode.valueOf(tag.getStringOr("Sort", "")); }
        catch (IllegalArgumentException ignored) { sort = SortMode.NAME_ASCENDING; }
        return new CategoryDefinition(id, tag.getStringOr("Name", ""), icon, tag.getIntOr("Order", 0),
                loadRules(tag.getListOrEmpty("Includes")), loadRules(tag.getListOrEmpty("Excludes")),
                Math.max(-1, tag.getLongOr("PickupLimit", 0L)), sort, tag.getBooleanOr("Enabled", false), tag.getBooleanOr("AllItems", false));
    }

    private static List<CategoryRule> loadRules(ListTag list) {
        ArrayList<CategoryRule> rules = new ArrayList<>();
        for (int i = 0; i < list.size(); i++) {
            String encoded = list.getStringOr(i, "");
            ...
            Identifier target = type == CategoryRule.Type.MOD_ID
                    ? Identifier.tryBuild(encoded.substring(1), "mod")
                    : Identifier.tryParse(type == CategoryRule.Type.TAG ? encoded.substring(1) : encoded);
