...
public final class InventoryCompatibilityGameTests {
    ...
    public static void clearVisitsHiddenLogicalStorage(GameTestHelper helper) {
        ...
        int removed = player.getInventory().clearOrCountMatchingItems(
                stack -> stack.is(Items.STONE), false, -1, new SimpleContainer(0));
    ...
    public static void creativePickAndPlacementUseLogicalStorage(GameTestHelper helper) {
        ...
        player.getInventory().addAndPickItem(new ItemStack(Items.STONE));

        helper.assertTrue(player.getInventory().getSelectedItem().is(Items.STONE),
    ...
    public static void recipeInventoryHooksDetectAndExtractStowedIngredients(GameTestHelper helper) {
        ...
        RecipeHolder<?> holder = helper.getLevel().getServer().getRecipeManager()
                .byKey(net.minecraft.resources.ResourceKey.create(net.minecraft.core.registries.Registries.RECIPE, Identifier.withDefaultNamespace("stick"))).orElseThrow();
        ...
        int sourceSlot = player.getInventory().findSlotMatchingCraftingIngredient(Items.OAK_PLANKS.builtInRegistryHolder(), ItemStack.EMPTY);
        ...
        sourceSlot = player.getInventory().findSlotMatchingCraftingIngredient(Items.OAK_PLANKS.builtInRegistryHolder(), ItemStack.EMPTY);
        helper.assertValueEqual(sourceSlot, Integer.MAX_VALUE,
    ...
    public static void nativeItemCapabilitySeesBackendAndRollsBack(GameTestHelper helper) {
        Player player = migratedPlayer(helper, GameType.SURVIVAL);
        var inventory = player.getData(ModAttachments.PLAYER_DATA).inventory();
        inventory.replaceSyntheticSlotFromItemUse(40, new ItemStack(Items.STONE, 7));
        var handler = player.getCapability(net.neoforged.neoforge.capabilities.Capabilities.Item.ENTITY);
        helper.assertTrue(handler instanceof com.cappleapple.bundlednotsiloed.compat.DynamicPlayerResourceHandler,
                "The player capability must use the logical inventory handler");
        helper.assertTrue(handler.getAmountAsLong(40) == 7, "Capability must see hidden storage");
        ...
        try (var transaction = net.neoforged.neoforge.transfer.transaction.Transaction.openRoot()) {
            helper.assertTrue(handler.extract(40, stone, 3, transaction) == 3, "Capability extracts hidden stack");
            helper.assertTrue(inventory.syntheticStack(40).getCount() == 4, "Transaction exposes tentative quantity");
        }
        helper.assertTrue(inventory.syntheticStack(40).getCount() == 7, "Aborted transaction restores hidden stack");
        try (var transaction = net.neoforged.neoforge.transfer.transaction.Transaction.openRoot()) {
            handler.extract(40, stone, 3, transaction);
            transaction.commit();
        }
        helper.assertTrue(inventory.syntheticStack(40).getCount() == 4, "Committed transaction keeps extraction");
        inventory.replaceSyntheticSlotFromItemUse(0, new ItemStack(Items.STONE, 12));
        long beforeRollback = inventory.revision();
        try (var transaction = net.neoforged.neoforge.transfer.transaction.Transaction.openRoot()) {
            helper.assertTrue(handler.extract(0, stone, 5, transaction) == 5, "Capability extracts a visible stack");
        }
        helper.assertTrue(player.getInventory().getNonEquipmentItems().getFirst().getCount() == 12,
                "Rollback must restore the vanilla compatibility stack reference");
        player.getData(ModAttachments.PLAYER_DATA).reconcileVanillaCompatibilityView();
        helper.assertTrue(inventory.syntheticStack(0).getCount() == 12 && inventory.revision() == beforeRollback,
                "Vanilla reconciliation must not undo an aborted transaction");
        try (var transaction = net.neoforged.neoforge.transfer.transaction.Transaction.openRoot()) {
            helper.assertTrue(handler.insert(1, stone, 6, transaction) == 6, "Capability inserts into an empty visible slot");
            player.getData(ModAttachments.PLAYER_DATA).reconcileVanillaCompatibilityView();
            helper.assertTrue(inventory.syntheticStack(1).getCount() == 6,
                    "Vanilla reconciliation must preserve a tentative insertion");
        }
        helper.assertTrue(inventory.syntheticStack(1).isEmpty()
