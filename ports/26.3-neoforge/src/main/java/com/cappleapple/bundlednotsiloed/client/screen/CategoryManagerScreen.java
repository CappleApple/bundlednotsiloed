...
public final class CategoryManagerScreen extends Screen {
    ...
    private void rebuildButtons() {
        ...
        addRenderableWidget(Button.builder(Component.translatable("gui.bundlednotsiloed.add_tab"), ignored ->
                        minecraft.gui.setScreen(new CategoryEditorScreen(this, player, null)))
        ...
        addRenderableWidget(Button.builder(Component.translatable("gui.bundlednotsiloed.reset_defaults"), ignored ->
                        net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(new CategoryEditPayload(CategoryEditPayload.Operation.RESET, new net.minecraft.nbt.CompoundTag())))
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        for (Map.Entry<Button, CategoryDefinition> entry : iconButtons.entrySet()) {
            ...
            graphics.item(CategoryIcons.displayStack(entry.getValue()), button.getX() + (button.getWidth() - 16) / 2, button.getY() + 2);
        }
    }
    ...
    private void bindSelected(CategoryDefinition category) {
        ...
        net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(new HotbarBindPayload(selectedHotbar, binding.type(),
    ...
    private void remove(CategoryDefinition category) {
        ...
        net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(new CategoryEditPayload(CategoryEditPayload.Operation.DELETE, tag));
    }
    ...
    private static void send(CategoryDefinition category) {
        net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(new CategoryEditPayload(CategoryEditPayload.Operation.UPSERT, PlayerCategoryData.saveCategory(category)));
    }
}
