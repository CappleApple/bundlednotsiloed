...
public final class ClientInventoryWindows {
    ...
    public static void open() {
        ...
        net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(new BrowserStatePayload(true, state.session(), state.containerId()));
    }
    public static void close() {
        if (state == null) return;
        net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(new BrowserStatePayload(false, state.session(), state.containerId()));
        reset();
    }
    ...
    private static void sendDesired() {
        ...
        if (request != null) net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(request);
    }
