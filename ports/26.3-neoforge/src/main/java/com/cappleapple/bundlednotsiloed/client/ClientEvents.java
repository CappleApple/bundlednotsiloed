...
public final class ClientEvents {
    ...
    public static void awaitWindowKey(ScreenEvent.KeyPressed.Pre event) {
        if (ClientInventoryWindows.pending() && !InventorySearchInputCapture.isTyping() && event.getKey() != com.mojang.blaze3d.platform.InputConstants.KEY_ESCAPE) event.setCanceled(true);
    }
    ...
    public static void clientTick(ClientTickEvent.Post event) {
        ...
        while (ClientKeyMappings.CYCLE_FORWARD.consumeClick()) {
            net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(new HotbarCyclePayload(minecraft.player.getInventory().getSelectedSlot(), 1));
        }
        ...
        while (ClientKeyMappings.DUMP_TO_CONTAINER.consumeClick()) {
            net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(new BulkTransferPayload(BulkTransferPayload.Direction.TO_CONTAINER, target));
        }
        while (ClientKeyMappings.EXTRACT_FROM_CONTAINER.consumeClick()) {
            net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(new BulkTransferPayload(BulkTransferPayload.Direction.FROM_CONTAINER, target));
        }
        while (ClientKeyMappings.TOGGLE_AUTO_REFILL.consumeClick()) {
            ...
            net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(new AutoRefillPayload(enabled));
    ...
    @SubscribeEvent(priority = EventPriority.LOWEST)
    public static void renderContainerOverlay(ScreenEvent.Render.Foreground event) {
    ...
    @SubscribeEvent
    public static void renderFullInventoryBarriers(ScreenEvent.Render.Foreground event) {
